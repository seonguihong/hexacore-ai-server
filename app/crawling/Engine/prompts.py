...
3. keywords:
   ...
   - 문자열 배열 형태
4. published_at:
    - 작성자가 작성한 날짜를 raw_text에서 추출
    - 'YYYY.MM.DD HH:MM' 또는 'YYYY-MM-DD HH:MM' 형태만 유효
    - 시간 정보가 없으면 'YYYY.MM.DD' 또는 'YYYY-MM-DD' 형태만 사용
    - 가격, 지수, 거래량 등의 숫자와 혼동 금지
    - 여러 날짜가 등장하면 '게시글 작성일자'에 해당하는 첫 번째 날짜를 선택
[분석 대상 raw_text]
{text}
